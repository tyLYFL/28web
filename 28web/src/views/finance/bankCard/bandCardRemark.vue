...
import  { aflcBankCardCustsvcAdd } from '@/api/finance/bankCard.js'
    export default{
        name:'bandCardRemark',
        components:{
        ...
        methods: {
            ...
            handleSearch(type){
                switch(type){
                    ...
                    case 'sure':
                        if(this.form.remark){
                            let remakeForm = Object.assign({},this.form,{bankCardId:this.MarkObject.bankCardId,accountId:this.MarkObject.accountId});
                            aflcBankCardCustsvcAdd(remakeForm).then(res => {
                                // console.log('ad',res)
                                this.$message({
                                    type: 'success',
                                    message: '添加备注成功！'
