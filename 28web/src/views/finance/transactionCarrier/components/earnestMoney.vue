<template>
    <div class="identicalStyle" style="height:100%" v-loading="loading">
          ...
          <div class="classify_info">
            <div class="btns_box">
            专线承运商名称：<span class="pr50 titleName">{{agencyOrLineName?agencyOrLineName:''}}</span>剩余保证金：<span class="pr50">{{collateral?collateral:'0.00'}}</span>状态：<span :class="['pr50',statusValue>collateral?'transationRed':'transationBlue']">{{statusValue>collateral?"未缴清":"已缴清"}}</span>
  ...
  methods: {
    ...
    importExcel(){
        ...
        data_aflcMywalletDetailListExcel(0, 0,this.formAllData).then(res=>{
            ...
            NewDom.setAttribute('download', '专线承运商账户概况保证金.xls');
