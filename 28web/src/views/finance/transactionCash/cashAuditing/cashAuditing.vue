<template>
    <div class="identicalStyle " style="height:100%"  v-loading="loading">
          <el-form :inline="true" class="demo-ruleForm classify_searchinfo">
             <el-form-item label="用户姓名">
              ...
              <el-form-item label="用户账号">
               ...
               <el-form-item label="处理结果">
                    <el-select v-model="formAllData.auditOpinion" placeholder="请选择" clearable>
                        <el-option
                            v-for="item in auditOpinionList"
                            :key="item.code"
                            :label="item.name"
                            :value="item.code"
                            :disabled="item.disabled"
               ...
               <el-form-item label="提现结果">
                    <el-select v-model="formAllData.extractStatus" placeholder="请选择" clearable>
                        <el-option
                            v-for="item in extractStatusList"
                            :key="item.code"
                            :label="item.name"
                            :value="item.code"
                            :disabled="item.disabled"
            ...
            <div class="info_news">    
             <div class="btns_box">
                 <el-button type="primary" icon="el-icon-circle-check" plain :size="btnsize" @click="handleClick('edit')" v-has:FINANCE_WITHDRAW_MANAGE_WITHDRAW_AUDIT_ALL_LIST_AUTH>审核</el-button>
            </div> 
           ...
           <el-table-column label="序号" sortable  width="60">
            ...
            <template  slot-scope="scope">
            <span class="needMoreInfo" @click="pushOrderSerial(scope.row)">{{ scope.row.mobile}}</span>
            </template>
            </el-table-column>
         ...
         <!-- 页码 -->
     <div class="info_tab_footer">共计:{{ dataTotal }} <div class="show_pager"> <Pager :total="dataTotal" @change="handlePageChange" ref="pager" :sizes="sizes" :pageLog="page"/></div> </div>   
          </div>
    	</div> 
         <cashAuditingDetail
            :editType="editType"
...
export default {
  data(){
    return{
      ...
      dataTotal:null,    
      pagesize:20,//每页显示数
      page:this.$store.state.order.cashAuditingPage,//当前页      
      sizes:[20,50,100,400],
     }
  },
    components:{
