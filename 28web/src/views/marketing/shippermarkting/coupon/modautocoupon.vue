<template>
     <div class="newcouponBox commoncss">
      ...
      <el-dialog  :visible="dialogFormVisible_add" :before-close="change" :title="btntitle" modal-append-to-body :closeOnClickModal='false'>
        <el-form :model="formAllData" ref="formAllData" :rules="rulesForm">
          <el-row v-if="editType=='one'">
            <el-col>
               <el-form-item  label="触发条件：" :label-width="formLabelWidth" prop="activityType">
                    <el-radio-group v-model="formAllData.activityType" size="small" :disabled="disabledStatus">
            ...
            <el-col :span="10">
               <el-form-item  label="活动名称：" :label-width="formLabelWidth" prop="activityName">
                   <el-input v-model="formAllData.activityName" maxlength="20" :disabled="disabledStatus"></el-input>
               </el-form-item>  
            </el-col>
            <el-col :span="14">
          ...
          <el-row >
            <el-col>
                <el-form-item  label="发放区域：" :label-width="formLabelWidth" v-if="editType !=='add'&&!selectFlag" >
                    <el-input v-model="formAllData.areaName1" @focus="changeSelect" :disabled="disabledStatus"></el-input>
               </el-form-item>  
               <el-form-item  label="发放区域：" :label-width="formLabelWidth"  prop="areaName" v-else> 
         ...
         <el-row >
            ...
            <div class="ht_table" v-for="(form,keys) in formAllData.aflcCouponList" :key='keys'>
             ...
             <div class="ht_table_th1"><span  class="addItem" @click="addItem" v-if="!disabledStatus"> </span></div>
             ...
             <div class="ht_table_td1"><span  class="reduceItem" @click="reduceItem(keys)" v-if="!disabledStatus"> </span></div>
             <div class="ht_table_td table_th1"><el-input v-model="formAllData.aflcCouponList[keys].couponNum" maxlength="2" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'" v-numberOnly></el-input></div>
             <div class="ht_table_td table_th2"><el-input v-model="formAllData.aflcCouponList[keys].couponName" maxlength="20" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'"></el-input></div>
             <div class="ht_table_td table_th3">
                     <el-select v-model="formAllData.aflcCouponList[keys].couponType" clearable placeholder="请选择" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'">
                         <el-option
             ...
             <div class="ht_table_td table_th4"><el-input v-model="formAllData.aflcCouponList[keys].remissionDiscount" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'" v-numberOnly></el-input></div> 
             <div class="ht_table_td table_th5"><el-input v-model.number="formAllData.aflcCouponList[keys].conditionDeduction" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'" v-numberOnly></el-input></div>
             <div class="ht_table_td table_th6">在线支付</div>
             <div class="ht_table_td table_th7">
                     <el-select v-model="formAllData.aflcCouponList[keys].timeType" clearable placeholder="请选择" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'">
                         <el-option
                 ...
                 <span v-if="formAllData.aflcCouponList[keys].timeType =='AF046301'">/</span>
                     <span v-else>
                         <el-date-picker v-model="formAllData.aflcCouponList[keys].startTime"
                                :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'"
                                type="datetime"
                                value-format="timestamp"
             ...
             <div class="ht_table_td table_th9">
                     <span v-if="formAllData.aflcCouponList[keys].timeType =='AF046301'">
                         <el-input v-model="formAllData.aflcCouponList[keys].overTime" placeholder="过期天数" maxlength="3" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'"></el-input>
                     </span>
                     <span v-else>
                               <el-date-picker
             ...
             <div class="ht_table_td table_th10">
                         <el-select v-model="formAllData.aflcCouponList[keys].serivceCode" clearable placeholder="请选择" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'" @change="serivceObject(keys,$event)">
                          <el-option
             ...
             <div class="ht_table_td table_th11">
                  <el-tooltip class="item" effect="dark" :content="formAllData.aflcCouponList[keys].carTypeName" placement="top-start" :disabled="!formAllData.aflcCouponList[keys].carTypeName">
                    <el-select v-model="formAllData.aflcCouponList[keys].carType" clearable placeholder="请选择" multiple @change='changcarType' @focus='focuscarType(keys)' :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'||formAllData.aflcCouponList[keys].serivceCode=='AF01702'">
                          <el-option
             ...
             <div class="ht_table_td table_th12" @click="changeInput(keys,$event)">
                <span v-if="editType=='types'">
                    <el-input placeholder="" v-model="formAllData.aflcCouponList[keys].area"  disabled></el-input>
                </span>
                <span v-else>
                <el-input placeholder="" v-model="formAllData.aflcCouponList[keys].area"  id="inputArea" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'"></el-input>
                <el-tooltip class="item" effect="dark" :content="formAllData.aflcCouponList[keys].area" placement="top-start" :disabled="!formAllData.aflcCouponList[keys].area">
                <div class="moke" @click='open(formAllData.aflcCouponList[keys].ifIssueCoupon)' id="moke"></div>
                </el-tooltip>
                </span>
              </div>
             <div class="ht_table_td table_th13">
                     <el-select v-model="formAllData.aflcCouponList[keys].ifvouchersuperposition" clearable placeholder="请选择" :disabled="formAllData.aflcCouponList[keys].ifIssueCoupon=='1'">
                         <el-option
...
import {data_get_couponActive_list,data_get_couponActive_create,data_couponActive,data_couponActiveTime,data_get_couponActive_Id,data_get_couponActive2_Id,data_get_couponActive_update,data_get_aflcCoupon_add,data_get_aflcCouponUse_list} from '@/api/marketing/shippermarkting/couponActive.js'
...
export default {
  ...
  methods:{
    ...
    openDialog:function(){
          ...
          data_get_couponActive_Id(this.params[0].id).then((res)=>{
            ...
            data_get_aflcCouponUse_list(1,10,{activityId:this.params[0].id}).then(res=>{
      ...
      data_get_couponActive2_Id(this.params[0].id).then((res)=>{
          ...
          for(var i= 0;i<this.formAllData.aflcCouponList.length;i++){
              let carString= []
                 if(res.data[i].carType.length>7){
                     carString =  res.data[i].carType.split(',')
                 }
                 else{
                   carString =  res.data[i].carType.split(' ')
                 }
              ...
              this.formAllData.aflcCouponList[i].carType = carString
             ...
             let reg3=/^([1-9]|1[0-9]|20)(\.\d)?$/  //输入0到10
                for(var i=0;i<this.formAllData.aflcCouponList.length;i++){
                  if(!this.formAllData.aflcCouponList[i].couponNum){
                     this.$message.warning('第'+(i+1)+'条派发数量都不能为空');
                     return false
                   }
                  ...
                  if(!this.formAllData.aflcCouponList[i].remissionDiscount){
                     this.$message.warning('第'+(i+1)+'条满减/折扣都不能为空');
                     return false
                   }
    ...
    changcarType(e){
     ...
     add_data(){
            ...
            this.$refs['formAllData'].validate(valid=>{
              ...
              this.$emit('getData');
                    if(res.text='已发放过优惠券不能进行修改')
                    {
                    this.$message.error('已发放过优惠券不能进行修改')
                    }
                    else{
  ...
  mounted(){
    ...
    if(e.target.id=='checkboxCityList'||e.target.id=="moke"&&_this.disabledStatus !==1)
    {
