<template>
    <div class="identicalStyle RecommendUsersOrderTotal" v-loading="loading">
        ...
        <div class="classify_info">
            <div class="btns_box">
                <el-button type="primary" icon="el-icon-download" plain :size="btnsize" @click="handleClick('copy')" v-has:MARKETING_DISTRIBUTION_STATISTICAL_RECOMMENDED_USEORDER_SUM_EXPORT_EXCEL>导出EXCEL</el-button>
			</div>
            <div class="info_news">
                <el-table
                :data="tableData" 
...
import { RecommendUsersOrderTotal } from '@/api/marketing/distribution'
import Pager from '@/components/Pagination/index'
import { objectMerge2,parseTime } from '@/utils/'
import { SaveAsFile } from '@/utils/lodopFuncs'

export default{
    name:'RecommendUsersOrderTotal',
    components: {
        Pager,
    },
    data() {
        return {
            ...
            tableColumn:[{
                ...
                'label': '货主推荐人',
                'prop': 'shipperRecommenderName',
                width:'200',
                fixed: false,
                overflow:true,
            },{
                'label': '货主推荐人用户类型',
                'prop': 'shipperRecommenderUserTypeName',
                width:'150',
                fixed: false,
                overflow:true,
            },{
                'label': '货主推荐人所属的业务员',
                'prop': 'shipperRecommenderSalesmanName',
                width:'200',
                fixed: false,
                overflow:true,
            },{
                'label': '车主推荐人',
                'prop': 'driverRecommenderName',
                width:'200',
                fixed: false,
                overflow:true,
            },{
                'label': '车主推荐人用户类型',
                'prop': 'driverRecommenderUserTypeName',
                width:'150',
                fixed: false,
                overflow:true,
            },{
                'label': '车主推荐人所属的业务员',
                'prop': 'driverRecommenderSalesmanName',
                width:'150',
                fixed: false,
                overflow:true,
            },{
    ...
    methods: {
        ...
        firstblood() {
            this.loading = true;
            RecommendUsersOrderTotal(this.listSystemObj).then(res => {
        ...
        handleClick(){
            SaveAsFile({
                ...
                name: '推荐用户订单汇总-' + parseTime(new Date(), '{y}{m}{d}{h}{i}{s}')
            })
        }
    }
}
