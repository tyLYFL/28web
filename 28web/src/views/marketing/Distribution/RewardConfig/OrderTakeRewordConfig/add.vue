<template>
    <div class="OrderTakeRewordConfigADD commoncss rewordStyle">
        <el-dialog :title='formtitle' :close-on-click-modal="false" v-el-drag-dialog top="5vh" :visible="dialogFormVisible" @close="close">
            <el-form   :model="standForm" :rules="newrules" label-position="top"  ref="ruleForm"  :label-width="formLabelWidth">
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="每个接单码奖励上限：" prop="perOrderRewardLimit">
                            <el-input v-model.number="standForm.perOrderRewardLimit" auto-complete="off" v-numberOnly maxlength="6"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item  label="每个接单码每天奖励次数上限：" prop="perOrderDayRewardTimeLimit">
                ...
                <el-row :gutter="20">
                    <el-col :span="12">
                        <el-form-item label="同一货主每天扫接单码奖励单数上限：" prop="perUserDayRewardTimeLimit">
                            <el-input v-model.number="standForm.perUserDayRewardTimeLimit" auto-complete="off" maxlength="3" v-numberOnly></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item  label="同一货主扫同一接单码奖励单数上限：" prop="perUserOrderRewardTimeLimit">
...
export default {
    ...
    data() {
        ...
        var checkNumberA = (rule, value, callback) => {
            if (!value) {
                return callback(new Error('每个接单码奖励上限不能为空'));
            }
            else if (!Number.isInteger(value)) {
                callback(new Error('请输入数字值'));
            } else {
        ...
        var checkNumberB = (rule, value, callback) => {
            if (!value) {
                return callback(new Error('每个接单码每天奖励次数上限不能为空'));
            }
            else if (!Number.isInteger(value)) {
                callback(new Error('请输入数字值'));
            } else {
        ...
        var checkNumberC = (rule, value, callback) => {
            if (!value) {
                return callback(new Error('同一货主每天扫接单码奖励单数上限不能为空'));
            }
            else if (!Number.isInteger(value)) {
                callback(new Error('请输入数字值'));
            } else {
        ...
        var checkNumberD = (rule, value, callback) => {
            if (!value) {
                return callback(new Error('同一货主扫同一接单码奖励单数上限不能为空'));
            }
            else if (!Number.isInteger(value)) {
                callback(new Error('请输入数字值'));
            } else {
      ...
      return {
        ...
        optionsServicetype:[],
        remarkList:[{
            label:'接单码鼓励车主分发，奖励车主；'
        },{
            label:'活动奖励总上限：当前配置的活动，所有使用接单码的车主获取的总奖励金额不能大于该数值；超出该数值时活动奖励自动熔断，不再发放奖励；'
        },{
            label:'每个接单码奖励上限：当前配置的活动中，每个接单码最多可获得的奖励上限；活动期内单个接单码奖励不能超出该数值；'
        },{
            label:'单个接单码每天奖励单数上限：当前配置的活动中，每个接单码每天可获得奖励次数的上限；当天单个接单码奖励次数不能大于该数值；'
        },{
            label:'同一货主每天扫接单码奖励单数上限：当前配置的活动中，同一货主当天扫描一个或多个接单码下单，奖励接单码的单数不能大于该数值，防止同一货主给多个接单码刷奖励；'
        },{
            label:'同一货主扫同一接单码奖励单数上限：当前配置的活动中，同一货主扫描同一接单码下单奖励发放的次数；超出该数值后，货主扫码下单，接单码不能得到奖励，防止同一货主利用同一接单码刷单。'
        }]
      };
    },
    watch:{
    ...
    methods: {
                ...
                if (valid) {
                    ...
                    this.standForm.aflcRecommendAwardList.forEach(el => {
                        if(!el.awardStart || !el.awardEnd){
                            isWrong = true;
                            messageTip = '请填写接单码完整奖励金额数据!'
                        }else if(!(el.recomAuthStart+ '')){
                            isWrong = true;
                            messageTip = '请完善接单码奖励单数区间!'
                        }
                    })
        ...
        addItem(idx){
            this.standForm.aflcRecommendAwardList.push({
                ...
                recomAuthEnd:null,//单数结束
                awardStart:null,//奖励结束金额
                awardEnd:null,//奖励结束金额
            }); 
        },
        //删除子节点新增
        reduceItem(idx){
