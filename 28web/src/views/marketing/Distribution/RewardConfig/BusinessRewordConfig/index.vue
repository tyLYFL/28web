    ...
    <div class="identicalStyle RecommendReword" v-loading="loading">
        <el-form :inline="true" :model="listSystemObj" ref="ruleForm" class="demo-ruleForm classify_searchinfo">
            ...
            <el-form-item label="奖励类型" prop="rewardTypeCode">
                <el-select v-model="listSystemObj.vo.rewardTypeCode" clearable placeholder="请选择" >
                    <el-option
        ...
        <div class="classify_info">
            <div class="btns_box">
                <el-button type="primary" icon="el-icon-circle-plus" plain :size="btnsize" @click="handleClick('new')" v-has:MARKETING_DISTRIBUTION_REWARD_BUSINESS_REWARD_ADD>新增</el-button>
			</div>
            <div class="info_news">
                <el-table
                :data="tableData"
                ...
                height="100%"   
                tooltip-effect="dark"
                v-has:MARKETING_DISTRIBUTION_REWARD_BUSINESS_REWARD_LIST
                style="width: 100%"
                >
                    ...
                    <el-table-column
                        ...
                        >
                        <template slot-scope="scope">
                            <!-- <p v-if="scope.row.aflcRecommendAwardList.length == 1">
                    ...
                    <el-table-column label="操作" fixed="right" width="150">
                        <template slot-scope="scope">
                            ...
                            type="primary"
                            plain
                            @click="handleClick('revise',scope.row)" v-has:MARKETING_DISTRIBUTION_REWARD_BUSINESS_REWARD_UPDATE>修改</el-button>
                            <el-button
                            :size="btnsize"
                            :type="scope.row.flag == '0' ? 'primary' : 'info'"
                            :disabled="scope.row.id ? false : true"
                            plain
                            @click="handleClick('status',scope.row)" v-has:MARKETING_DISTRIBUTION_REWARD_BUSINESS_REWARD_USE>{{scope.row.flag == 0 ? '启用' : '禁用'}}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
        </div>   
        <div class="info_tab_footer" v-has:MARKETING_DISTRIBUTION_REWARD_BUSINESS_REWARD_LIST>共计:{{ totalCount }} <div class="show_pager"> <Pager :total="totalCount" @change="handlePageChange" ref="pager"/></div> </div>    
        <add :dialogFormVisible.sync = "dialogFormVisible" @renovate="Onrenovate" :changeForm="changeForm" :isModify="isModify" :formtitle = "formtitle"/>
    </div>
</template>

<script type="text/javascript">
import { BusinessRewordConfigList,BusinessRewordConfigStatus } from '@/api/marketing/distribution'
...
export default{
    ...
    methods: {
        ...
        firstblood() {
            this.loading = true;
            BusinessRewordConfigList(this.listSystemObj).then(res => {
        ...
        handleClick(type,row){
            switch(type){
                case 'new':
                    ...
                    this.isModify = false;
                    this.formtitle = '新增业务奖励配置'
                    break;
                case 'revise':
                    this.dialogFormVisible = true;
                    this.isModify = true;
                    this.formtitle = '修改业务奖励配置';
                    this.changeForm = row;
                    break;
                case 'status':
                    ...
                    this.$confirm(message, '提示', {
                        ...
                        let ids = [];
                        ids.push(row.id);
                        BusinessRewordConfigStatus(ids).then(res => {
                            this.$message({
                                type: 'success',
                                message: '操作成功！',
                            })
