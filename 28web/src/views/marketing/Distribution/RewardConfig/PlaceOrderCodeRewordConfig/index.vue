<template>
    <div class="identicalStyle placeOrderCodeConfig" v-loading="loading">
        ...
        <div class="classify_info">
            <div class="btns_box">
                <el-button type="primary" icon="el-icon-circle-plus" plain :size="btnsize" @click="handleClick('new')" v-has:MARKETING_DISTRIBUTION_REWARD_PLACE_ORDER_REWARD_ADD>新增</el-button>
			</div>
            <div class="info_news">
                <el-table
                :data="tableData" 
                ...
                height="100%"   
                tooltip-effect="dark"
                v-has:MARKETING_DISTRIBUTION_REWARD_PLACE_ORDER_REWARD_LIST
                style="width: 100%"
                >
                    ...
                    <el-table-column
                        ...
                        label="同一货主每天扫下单码奖励单数上限"
                        width="150"
                        >
                    </el-table-column>
                    <el-table-column
                        prop="perUserOrderRewardTimeLimit"
                        label="同一货主扫同一下单码奖励单数上限"
                        width="150"
                        >
                    </el-table-column>
                    <el-table-column
                    ...
                    <el-table-column label="操作" fixed="right" width="150">
                        <template slot-scope="scope">
                            ...
                            type="primary"
                            plain
                            @click="handleClick('revise',scope.row)" v-has:MARKETING_DISTRIBUTION_REWARD_PLACE_ORDER_REWARD_UPDATE>修改</el-button>
                            <el-button
                            :size="btnsize"
                            :type="scope.row.flag == '0' ? 'primary' : 'info'"
                            :disabled="scope.row.id ? false : true"
                            plain
                            v-has:MARKETING_DISTRIBUTION_REWARD_PLACE_ORDER_REWARD_USE
        ...
        <div class="info_tab_footer" v-has:MARKETING_DISTRIBUTION_REWARD_PLACE_ORDER_REWARD_LIST>共计:{{ totalCount }} <div class="show_pager"> <Pager :total="totalCount" @change="handlePageChange" ref="pager"/></div> </div>    
        ...
        handleClick(type,row){
            switch(type){
                case 'new':
                    ...
                    this.isModify = false;
                    this.formtitle = '新增下单码奖励配置'
                    break;
                case 'revise':
                    this.dialogFormVisible = true;
                    this.isModify = true;
                    this.formtitle = '修改下单码奖励配置';
                    this.changeForm = row;
                    break;
                case 'status':
