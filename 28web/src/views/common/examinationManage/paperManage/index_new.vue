            ...
            <div class="side_right">
                <div class="side_right_bottom clearfix">
                    <div class="btns_box clearfix">
                        <el-button type="primary" size="mini" plain icon="el-icon-circle-plus" @click="handleClick('add')" >新增考卷</el-button>
                    </div>
                    <div class="info_news">
                          ...
                          <el-table-column prop="standardPrice" sortable width="420" label="操作">
                            <template slot-scope="scope">
                              <div class="display_cell">
                                ...
                                <el-button type="primary" size="mini" v-if="scope.row.status === 'draft'" plain @click="handleListQuestion(scope.row.id)">选择题目</el-button>
        ...
        <el-dialog :close-on-click-modal="false" v-el-drag-dialog :visible.sync="dialogFormVisible" :title="btntitle" @close="closeClick" width="750px">
          <el-form ref="formData" :model="formData" label-position="right" label-width="130px">
            ...
            <el-form-item label="考卷状态" prop="status" :rules="{required: true, message: '选择考卷状态', trigger: 'change'}">
              <el-select style="width: 50%" disabled v-model="formData.status" clearable placeholder="请选择">
                <el-option
  ...
  methods: {
    handleUp(item) {
      if (item.totalnum === 0) {
        ...
        this.$confirm('是否上架，上架后不可修改与删除?', '提示', {
          ...
          fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/publishTestpaper?id=${item.id}`, method: 'post', data: {}}).then((res) => {
            if (res.status === 200) {
    ...
    handleDowm(item) {
      this.$confirm('是否下架?', '提示', {
        ...
        fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/offlineTestpaper?id=${item.id}`, method: 'post', data: {}}).then((res) => {
          if (res.status === 200) {
    ...
    submit() {
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          if (this.btntitle === '新增') {
            fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/add2`, method: 'post', data: this.formData }).then((res) => {
              if (res.status === 200) {
            ...
            fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/update2`, method: 'post', data: this.formData }).then((res) => {
              if (res.status === 200) {
    ...
    del(item) {
      this.$confirm('此操作将删除选择的数据, 是否确定删除?', '提示', {
        ...
        this.loading = true
        fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/delete2`, method: 'post', data: { id: item.id }}).then((res) => {
          if (res.status === 200) {
    ...
    submitListQuestion() {
      ...
      fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/updateQuestion2`, method: 'post', data: parm }).then((res) => {
        if (res.status === 200) {
    ...
    getListQuestion() {
      ...
      fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/listQuestion2?pageNum=${this.pageListQuestion}&pageSize=${this.pagesizeListQuestion}`, method: 'post', data: parm }).then((res) => {
        if (res.status === 200) {
    ...
    handlePreview(item) {
      ...
      const list = ['A.', 'B.', 'C.', 'D.', 'E.', 'F.', 'G.']
      fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/previewTestpaper2?id=${item.id}`, method: 'post', data: {}}).then((res) => {
        if (res.status === 200) {
          if (res.data.dan) {
    ...
    getList(parm = {}) {
      this.loading = true
      fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningtestpaper/v1/list2?pageNum=${this.page}&pageSize=${this.pagesize}`, method: 'post', data: parm }).then((res) => {
        if (res.status === 200) {
