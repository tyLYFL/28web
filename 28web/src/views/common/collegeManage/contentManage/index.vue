<template>
    <div class="TwoColumns clearfix" v-loading ="loading">
            ...
            <el-form :inline="true" ref="ruleForm" class="demo-ruleForm classify_searchinfo">
                ...
                <div class="side_right_bottom clearfix">
                    <div class="btns_box clearfix">
                        <el-button type="primary" size="mini" plain icon="el-icon-circle-plus" @click="handleClick('add')" v-has:SYSTEM_DRIVER_COLLEGE_MANAGE_COURSE_CONTENT_ADD>新增</el-button>
                    </div>
                    <div class="info_news">
                        <el-table ref="multipleTable" :data="tableData" :span-method="objectSpanMethod" stripe border height="100%" tooltip-effect="dark" style="width: 100%" v-has:SYSTEM_DRIVER_COLLEGE_MANAGE_COURSE_CONTENT_LIST>
                            <el-table-column label="序号" width="60">
                                <template slot-scope="scope">
                                    {{ scope.$index + 1 }}
                                </template>
                            </el-table-column>  
                            <el-table-column prop="areaName" sortable min-width="176" label="课程分类名称">
                              <template slot-scope="scope">{{scope.row.serviceidName }} - {{scope.row.courseid }}</template>
                            </el-table-column>
                            <el-table-column prop="title" sortable min-width="176" label="课程标题">
                            </el-table-column>
      ...
      <div class="commoncss">
        <el-dialog :close-on-click-modal="false" v-el-drag-dialog :title='btntitle' :visible.sync="dialogFormVisible" @close="closeClick" @open="openClick" width="1000px">
        <el-form ref="formData" :model="formData" label-position="right" label-width="120px" class="demo-ruleForm">
          <el-form-item label="选择课程分类" prop="classification" :rules="{required: true, message: '请选择课程分类', trigger: 'change'}">
            <el-cascader
              style="width: 50%"
              clearable
              expand-trigger="hover"
          ...
          <el-form-item label="课程标题" prop="title" :rules="{required: true, validator: (rule, value, callback)=>{
          return checkLength(rule, value, callback,'请输入最少5个字最多25个字课程标题')
          }, trigger: 'blur'}">
            <el-input style="width: 50%" v-model="formData.title" minlength="5" maxlength="25" clearable></el-input>
          </el-form-item>
          <el-form-item label="课程内容" class="editors" prop="content" :rules="{required: true, message: '请输入课程内容', trigger: 'blur'}">
            <div>
              <editor
                class="editor"
      ...
      <el-dialog :close-on-click-modal="true" v-el-drag-dialog title='预览课程内容' :visible.sync="dialogFormVisible1" width="500px">
  ...
  methods: {
    ...
    submit() {
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          if (this.btntitle === '新增') {
            fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningcourse/v1/add`, method: 'post', data: this.formData }).then((res) => {
              if (res.status === 200) {
            ...
            fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningcourse/v1/update?id=${this.formData.id}`, method: 'post', data: this.formData }).then((res) => {
              if (res.status === 200) {
    ...
    getList(parm = {}) {
      this.loading = true
      fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningcourse/v1/list?pageNum=${this.page}&pageSize=${this.pagesize}`, method: 'post', data: parm }).then((res) => {
        if (res.status === 200) {
    ...
    del(item) {
      this.$confirm('此操作将删除选择的数据, 是否确定删除?', '提示', {
        ...
        this.loading = true
        fetch({ url: `/${this.$cms}/usercenter/aflcdriverlearningcourse/v1/delete?id=${item.id}`, method: 'post', data: {}}).then((res) => {
          if (res.status === 200) {
