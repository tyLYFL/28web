<template>
    <div class="identicalStyle deliverPriceList" v-loading="loading">
...
import { collectFeesList,collectFeesStatus } from '@/api/server/lingdan/otherService.js'
		...
		handleClick(type,row){
            switch (type) {
                case 'new':
                    ...
                    this.formtitle = '新增送货费定价';
                    this.isModify = false;
                    this.isBatch = false;
                    break;
                case 'revise':
                    this.dialogFormVisible = true;
                    this.formtitle = '修改送货费定价';
                    this.isModify = true;
                    this.isBatch = false;
                    this.changeForm = row;
                    break;
                case 'batch':
                    ...
                    if(this.checkedinformation.length == 0){
                        ...
                        this.isModify = false;
                        this.isBatch = true;
                        this.formtitle = '批量修改送货费定价';
                    }
                    break;
                case 'status':
                    let message = row.flag == '0' ? '确认启用当前街道送货费定价吗?' :'确认禁用当前街道送货费定价吗？禁用后将不收取客户的送货费!';
                    this.$confirm(message, '提示', {
                        ...
                        collectFeesStatus(statusObj).then(res => {
                            this.firstblood()
                            this.$message({
                                type: 'success',
