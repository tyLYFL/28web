<template>
  <div class="newTradeAreaServicePrice carrierCommonStyle">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="200px" class="demo-ruleForm">
        <div class="searchInformation rangeCommon">
            <h2>基本信息</h2>
            <el-row :gutter="20">
                <el-col :span="10">
                    <el-form-item label="商圈所在地：" prop="areaName" class="location_line">
                 ...
                 <el-col :span="10">
                    <el-form-item label="商圈：" prop="tradeId" class="location_line">
                        <el-input v-model="ruleForm.tradeName" v-if="ifShowRangeType == '1'" disabled></el-input>
                        <el-select v-model="ruleForm.tradeId" :loading="loadingTrade" :no-data-text = "dataText" filterable placeholder="请选择商圈" @focus="getTradeList"  v-else>
                            <el-option
        ...
        <div class=" priceTime rangeCommon">
            ...
            <el-form-item label="送货费定价：" class="jieti" prop="" required>
                <div class="goodsPriceChoose">
                    <el-row class="goodsPriceChoose_title">
                        <el-col :span="8">标准货物分类数量区间</el-col>
                        <el-col :span="8">送货范围（公里）</el-col>
                        <el-col :span="8">送货费（元）</el-col>
                    </el-row>
                     <div v-for="(form,keys) in deliverGoods" :key="keys"  class="deliverGoods">
                        <div :span="6" class="intervalNum blackBook" style="width:33.33333%">
                            <el-input v-model.number="form.volumeInterval1" v-numberOnly placeholder="包含" maxlength="7" disabled></el-input>
                            <span>----</span>
                        ...
                        <div :span="12" class="intervalNum otherStyle" style="width:66.66666%">
...
import { aflcTradeAreaTradeList,newAflcTradeAreaServicePrice,TradeAreaServicePriceDetails } from '@/api/server/lingdan/tradeAreaServicePrice.js'
...
export default {
    ...
    data() {
        ...
        var checkStartLocation = (rule,value,callback) => {
            // console.log('this.ruleForm.startCity',this.ruleForm.province)
            if (value == '') {
                callback(new Error('请选择区域'));
            } else if(this.ruleForm.city == '' && this.ruleForm.province !=  '北京市' && this.ruleForm.province !=  '天津市' && this.ruleForm.province !=  '重庆市' && this.ruleForm.province !=  '上海市' ){
        ...
        return {
            ...
            //保价费定价
            supportGoods:[{
                costType:'3',//费用类型(保价费定价:3 ;代收货款手续费定价:4 ; 时效保障费定价:5)
            ...
            //代收货款手续费定价
            FeePricing:[{
                costType:'4',//费用类型(保价费定价:3 ;代收货款手续费定价:4 ; 时效保障费定价:5)
                costInterval1:'0',//干线费区间1
                costInterval2:'',//干线费区间2
                lowestCost:'',//最低一票收费
                highestCost:'',//最高一票收费
            ...
            //时效保障费定价
            timeValue:[{
                costType:'5',//费用类型(保价费定价:3 ;代收货款手续费定价:4 ; 时效保障费定价:5)
                costInterval1:'0',//干线费区间1
                costInterval2:'',//干线费区间2
                lowestCost:'',//最低一票收费
                highestCost:'',//最高一票收费
    ...
    methods:{
        ...
        getTradeList(){
            ...
            if(this.ruleForm.areaCode == ''){
                ...
                aflcTradeAreaTradeList(this.ruleForm.areaCode).then(res => {
                    setTimeout(() => {
                        this.loadingTrade = false;
                        this.optionsTrade = res.data;
                        if(this.optionsTrade == [] && this.ruleForm.tradeId){
                            this.ruleForm.tradeId = ''
                        }
                    }, 200);
                }).catch(err => {
                    this.ruleForm.tradeId = '';
        ...
        getParams(){
            if(this.$route.query.tradeId){
                ...
                // console.log('```',tradeId,this.ifShowRangeType)
                TradeAreaServicePriceDetails(tradeId).then(res=>{
                    ...
                    this.takeGoods.sort(function(a, b) {
                        return a.volumeInterval1 - b.volumeInterval1
                    })
            ...
            switch(type){
                ...
                case 'supportGoods':
                        this.supportGoods.push({
                            costType:'3',//费用类型(保价费定价:0 ;代收货款手续费定价:1 ; 时效保障费定价:2)
                ...
                case 'FeePricing':
                        this.FeePricing.push({
                            costType:'4',//费用类型(保价费定价:0 ;代收货款手续费定价:1 ; 时效保障费定价:2)
                ...
                case 'timeValue':
                         this.timeValue.push({
                            costType:'5',//费用类型(保价费定价:0 ;代收货款手续费定价:1 ; 时效保障费定价:2)
        ...
        completeName(){
            ...
            if(this.ifShowRangeType == '0'){
                this.ruleForm.tradeName = this.optionsTrade.find(item => item.tradeId ===  this.ruleForm.tradeId)['tradeName'];
            }
        },
        //提交按钮
        submitForm(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.deliverGoods.forEach(item => {
                        if((item.volumeInterval1 + '') == ''){
                            messageInfo= '送货费区间最小值不可为空' 
                            ifNull = false;
                        }else{
                            item.deliverIntervalList.forEach(el => {
                                if(el.deliverInterval1 == '' && el.deliverInterval1 != 0){
                    ...
                    this.takeGoods.forEach(item => {
                        console.log(item.volumeInterval1)
                        if((item.volumeInterval1 + '') == ''){
                    ...
                    if(ifNull){
                        this.completeName();
                        let commitFunction = newAflcTradeAreaServicePrice(this.ruleForm);
                        commitFunction.then(res => {
                            if(res.status == 200){ 
                                this.$alert('操作成功', '提示', {
                                    confirmButtonText: '确定',
                                    callback: action => {
                                        sessionStorage.setItem('tradeAreaCarrierName','tradeAreaServicePrice');
