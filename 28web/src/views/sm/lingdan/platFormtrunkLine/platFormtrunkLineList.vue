<template>
    <div class="identicalStyle trunkLinePriceList" v-loading="loading">
        ...
        <div class="classify_info">
            <div class="btns_box">
                <el-button type="primary" icon="el-icon-circle-plus" plain @click="handleClick('new')" :size="btnsize">新增标准定价</el-button>
                <el-button type="primary" icon="el-icon-upload2" plain @click="handleClick('batchImport')" :size="btnsize">批量导入标准定价</el-button>
                <el-button type="primary" icon="el-icon-upload" plain @click="handleClick('export')" :size="btnsize">导出标准定价数据</el-button>
...
import add from './newPlatFormtrunkLine'
		...
		handleClick(type,row){
            switch (type) {
                case 'new':
                    ...
                    this.formtitle = '新增平台线路标准定价';
                    this.isModify = false;
                    this.isBatch = false;
                    break;
                case 'revise':
                    this.dialogFormVisible = true;
                    this.formtitle = '修改平台线路标准定价';
                    this.isModify = true;
                    this.isBatch = false;
                    this.changeForm = row;
                    break;
                ...
                //     break;
                case 'status':
                    let message = row.flag == '0' ? '确认启用当前路线定价吗?' :'确认禁用当前路线定价吗？禁用后使用平台标准定价的区代将不能收到当前路线的订单。';
