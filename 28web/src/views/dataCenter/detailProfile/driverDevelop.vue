<template>
    <div class="identicalStyle clearfix detailProfile" v-loading="loading">
      ...
      <div class="TotalH2">{{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-1 至 {{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-{{daycount}}车主发展概况
           <el-button type="primary" :size="btnsize" plain @click="importExcel" icon="el-icon-download" class="fixedExcel" v-has:DATA_CENTER_DEVELOPMENT_OVERVIEW_DRIVER_DEVELOP_GENERAL_EXCEL>导出Excel</el-button>
      </div>
      <div class="info_news">
            ...
            <el-table-column  label="汇总" prop="monthCount"  width="160" :render-header="renderHeader" id="moCount">
            </el-table-column>
...
import { data_post_findSalesmanDriverDevelSurveyList,data_post_exportSalesmanDriverDevelSurveyExcel } from '@/api/dataCenter/detailProfile'
...
import businessGroup from '@/components/selectTree/businessGroup'  
    export default{
        ...
        methods:{
            firstblood(){
            this.loading = true;
            data_post_findSalesmanDriverDevelSurveyList(this.page, this.pagesize,this.formAllData).then(res=>{
        ...
        data_post_exportSalesmanDriverDevelSurveyExcel(0, 0,this.formAllData).then(res=>{
