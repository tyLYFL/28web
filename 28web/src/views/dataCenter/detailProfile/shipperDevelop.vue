<template>
    <div class="identicalStyle clearfix detailProfile" v-loading="loading">
        ...
        <div class="TotalH2">{{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-1 至 {{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-{{daycount}}货主发展概况
             <el-button type="primary" :size="btnsize" plain @click="importExcel" icon="el-icon-download" class="fixedExcel" v-has:DATA_CENTER_DEVELOPMENT_OVERVIEW_SAHIPPER_DEVELOP_GENERAL_EXCEL>导出Excel</el-button>
        </div>
      <div class="info_news">
...
import { data_post_findSalesmanShipperDevelSurveyList,data_post_exportSalesmanShipperDevelSurveyExcel } from '@/api/dataCenter/detailProfile'
...
import businessGroup from '@/components/selectTree/businessGroup'
    export default{
        ...
        methods:{
            firstblood(){
            this.loading = true;
            data_post_findSalesmanShipperDevelSurveyList(this.page, this.pagesize,this.formAllData).then(res=>{
        ...
        importExcel(){
            ...
            data_post_exportSalesmanShipperDevelSurveyExcel(0, 0,this.formAllData).then(res=>{
                var blob = new Blob([res]);
                this.href =  window.URL.createObjectURL(blob);
                var NewDom = document.createElement("a");
                NewDom.setAttribute('href', this.href);
                NewDom.setAttribute('target', '_blank');
                NewDom.setAttribute('download', '货主发展概况'+ this.getYear + '年' + this.getMonth +'月.xls');
