<template>
    <div class="identicalStyle clearfix detailProfile" v-loading="loading">
        ...
        <el-form-item label="车主">
            <el-input v-model.trim="formAllData.driverName" clearable placeholder="请输入车主姓名"></el-input>
        </el-form-item>
        ...
        <div class="TotalH2">{{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-1 至 {{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-{{daycount}}车主接单概况
             <el-button type="primary" :size="btnsize" plain @click="importExcel" icon="el-icon-download" class="fixedExcel" v-has:DATA_CENTER_DEVELOPMENT_OVERVIEW_DRIVER_ORDER_GENERAL_EXCEL>导出Excel</el-button>
        </div>
      <div class="info_news">
            ...
            <el-table-column  label="车主姓名" prop="userName"  width="160" :render-header="renderHeader">
            </el-table-column>
...
import { data_post_findDriverOrderDevelSurveyList,data_post_exportDriverOrderDevelSurveyExcel } from '@/api/dataCenter/detailProfile'
...
import businessGroup from '@/components/selectTree/businessGroup'
    export default{
        ...
        methods:{
            firstblood(){
            this.loading = true;
            data_post_findDriverOrderDevelSurveyList(this.page, this.pagesize,this.formAllData).then(res=>{
        ...
        //   导出excel
          importExcel(){
            ...
            data_post_exportDriverOrderDevelSurveyExcel(0, 0,this.formAllData).then(res=>{
                var blob = new Blob([res]);
                this.href =  window.URL.createObjectURL(blob);
                var NewDom = document.createElement("a");
                NewDom.setAttribute('href', this.href);
                NewDom.setAttribute('target', '_blank');
                NewDom.setAttribute('download', '车主下单概况'+ this.getYear + '年' + this.getMonth +'月.xls');
