<template>
    <div class="identicalStyle clearfix detailProfile" v-loading="loading">
        ...
        <el-form-item label="货主">
               <el-input v-model.trim="formAllData.shipperName" clearable placeholder="请输入货主姓名"></el-input>
        </el-form-item>
        ...
        <div class="TotalH2">{{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-1 至 {{getYear}}-{{getMonth < 10?'0'+getMonth:getMonth}}-{{daycount}}货主下单概况
             <el-button type="primary" :size="btnsize" plain @click="importExcel" icon="el-icon-download" class="fixedExcel"  v-has:DATA_CENTER_DEVELOPMENT_OVERVIEW_SAHIPPER_ORDER_GENERAL_EXCEL>导出Excel</el-button>
        </div>
      <div class="info_news">
            ...
            <el-table-column  label="货主姓名" prop="userName"  width="160" :render-header="renderHeader" show-overflow-tooltip>
            </el-table-column>
            <el-table-column  label="手机号码" prop="userMobile"  width="160" :render-header="renderHeader" show-overflow-tooltip>
            </el-table-column>
...
import { data_post_findShipperOrderDevelSurveyList,data_post_exportShipperOrderDevelSurveyExcel } from '@/api/dataCenter/detailProfile'
...
import businessGroup from '@/components/selectTree/businessGroup'
    export default{
        ...
        methods:{
            firstblood(){
            this.loading = true;
            data_post_findShipperOrderDevelSurveyList(this.page, this.pagesize,this.formAllData).then(res=>{
        ...
        //   导出excel
          importExcel(){
            ...
            data_post_exportShipperOrderDevelSurveyExcel(0, 0,this.formAllData).then(res=>{
                var blob = new Blob([res]);
                this.href =  window.URL.createObjectURL(blob);
                var NewDom = document.createElement("a");
                NewDom.setAttribute('href', this.href);
                NewDom.setAttribute('target', '_blank');
                NewDom.setAttribute('download', '货主下单概况'+ this.getYear + '年' + this.getMonth +'月.xls');
        ...
        rowspan(data) {
                ...
                        }
                        ...
                        if (this.tableDataAll[i].userMobile === this.tableDataAll[i - 1].userMobile) {
                            this.userMobileArr[this.userMobile] += 1;
                            this.userMobileArr.push(0);
                        } else {
                            this.userMobileArr.push(1);
                            this.userMobile = i;
                        }
        ...
                    const _col = _row > 0 ? 1 : 0;
            ...
                }
                ...
                if (columnIndex === 2) {
                    const _row = this.userMobileArr[rowIndex];
                    const _col = _row > 0 ? 1 : 0;
                    return {
                        rowspan: _row,
                        colspan: _col
