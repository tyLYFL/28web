<template>
    <div class="identicalStyle agencyConfig" v-loading="loading">
        ...
        <addA :dialogFormVisible.sync = "dialogFormVisible" :isModify="isModify" :changeForm="changeForm" :formtitle = "formtitle"  @renovate="firstblood"/>
    </div>
</template>

<script type="text/javascript">
import { agencyConfigList,agencyConfigStatus } from '@/api/dispatch/plantformProfit'
import Pager from '@/components/Pagination/index'
// import add from './newTrunkLinePrice'
import addA from './component/addA'
export default{
    name:'agencyConfig',
	props: {
		isvisible: {
	...
	methods: {
		...
		firstblood() {
			this.loading = true
			agencyConfigList(this.listSystemObj).then(res => {
            ...
            if(this.selected.id || type =='new'){
                switch (type) {
                    case 'new':
                        ...
                        this.formtitle = '新增按区代配置平台分润规则';
                        this.isModify = false;
                        break;
                    case 'revise':
                        this.dialogFormVisible = true;
                        this.formtitle = '修改按区代配置平台分润规则';
                        this.isModify = true;
                        this.changeForm = this.selected;
                        break;
                    case 'status':
                        let message = this.selected.flag == '0' ? '确认启用当前区代配置吗?' :'确认禁用当前区代配置吗？禁用后区代配置将无效。';
                        this.$confirm(message, '提示', {
                            ...
                            agencyConfigStatus(statusObj).then(res => {
                                this.firstblood()
                                this.$message({
                                    type: 'success',
