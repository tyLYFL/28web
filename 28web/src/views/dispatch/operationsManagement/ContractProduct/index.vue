            ...
            <div class="info_news">
                <el-table
                    ...
                    <el-table-column
                        ...
                        label="产品名称">
                         <template slot-scope="scope">
                            <h4 class="needMoreInfo" @click="handleClick('details',scope.row)">{{ scope.row.productName}}</h4>
                        </template>
                    </el-table-column> 
                    <el-table-column
                        prop="typeName"
                    ...
                    <el-table-column
                        ...
                        sortable
                        label="截止日期">
                        <template slot-scope="scope">{{scope.row.endTime | parseTime('{y}-{m}-{d}')}}</template>
                    </el-table-column>
                    <el-table-column
                        prop="serviceInfo"
                        sortable
...
import { contractProductList,deleteContractProduct } from '@/api/dispatch/operationsManagement'
...
export default{
    ...
    methods: {
        ...
        firstblood() {
            this.loading = true
            contractProductList(this.listSystemObj).then(res => {
        ...
        handleClick(type,row){
            switch (type) {
                ...
                case 'new':
                    this.dialogFormVisible = true;
                    this.formtitle = '新增承包产品';
                    this.isModify = false;
                    this.isView = false;
                    break;
                case 'revise':
                    this.dialogFormVisible = true;
                    this.formtitle = '修改承包产品';
                    this.isModify = true;
                    this.isView = false;
                    this.changeForm = row;
                    break;
                case 'delet':
                    this.$confirm('确认要删除吗？', '提示', {
                        ...
                        let userId = row.id;
                        deleteContractProduct(userId).then(res => {
                            this.firstblood()
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
